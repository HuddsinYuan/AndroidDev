...
JNIEXPORT jint JNICALL Java_com_bjw_gpio_GPIOJNI_GetCfgpin
    ...
    //LOGD("fd_gpio is %d ,pin_group is %s,pin_num is %d", fd_gpio, pin_info.pin_group,
    //     pin_info.pin_num);
    if (fd_gpio < 0)
        return fd_err;
...
JNIEXPORT jint JNICALL Java_com_bjw_gpio_GPIOJNI_SetCfgpin
    ...
    //LOGD("fd_gpio is %d ,pin_group is %s,pin_num is %d,pin_cfg is %d", fd_gpio, cfg_data.pin_group,
    //     cfg_data.pin_num, cfg_data.pin_data);
    if (fd_gpio < 0)
        return fd_err;
...
JNIEXPORT jint JNICALL Java_com_bjw_gpio_GPIOJNI_SetPull
    ...
    //LOGD("fd_gpio is %d ,pin_group is %s,pin_num is %d,pin_cfg is %d", fd_gpio, pull_data.pin_group,
    //     pull_data.pin_num, pull_data.pin_data);
    if (fd_gpio < 0)
        return fd_err;
...
JNIEXPORT void JNICALL Java_com_bjw_gpio_GPIOJNI_uPause
        (JNIEnv *env, jclass thiz, jint time) {
    if (time > 0) {
        usleep(time);
    }
}
